...
barT = 5  # Timeout after being tagged
...
def Angle_Control(ti, MRobots, ORobots, z, tau, q, eta, MF, OF, NMA, NOA, dt):
    ...
    if q == 1 or eta==1:
        thetab = calculate_angle(MRobots.getZ()[0], MRobots.getZ()[1], MF[0], MF[1])
    # If activate, go to opponents base
    elif q==0:
        thetab = calculate_angle(MRobots.getZ()[0], MRobots.getZ()[1], OF[0], OF[1])
    # Else, sin
    else:
...
for ti in range(1,int(TSPAN[1]/dt)):
    ...
    for k in range (1,b+1):
        ...
        for i in range(1,r+1):
            ...
            if (pointInCircle(zB[0:2], FR[0:2], gf) and qB == 0 and etaB == 0 and muB ==1 ) \
                or (pointInCircle(zB[0:2], FB[0:2], gf) and qB == 0 and etaB == 1 and muB==0):
...
def update(i):
    ...
    robots = [
        ...
        ("Robot1", 'b', 'g', 'k', robot_scatter_1B, rect_1B, RobotsB),
        ("Robot2", 'b', 'g', 'k', robot_scatter_2B, rect_2B, RobotsB),
        ("Robot3", 'b', 'g', 'k', robot_scatter_3B, rect_3B, RobotsB),

        ("Robot1", 'r', 'm', 'k', robot_scatter_1R, rect_1R, RobotsR),
        ("Robot2", 'r', 'm', 'k', robot_scatter_2R, rect_2R, RobotsR),
        ("Robot3", 'r', 'm', 'k', robot_scatter_3R, rect_3R, RobotsR)
    ]
...
anim = animation.FuncAnimation(fig,update,frames=10000, interval=10)#(int(TSPAN[1]/dt)))
